...
const CartButton: React.FC<CartButtonProps> = ({ cartItemCount, onCartClick, isCartAnimating }) => {
  ...
  return (
    <div className="fixed w-80 h-80 top-[4%] left-[88%] z-40 flex flex-col gap-4 justify-center items-center">
      <button
        onClick={onCartClick}
        className={`relative px-8 py-6 rounded-full transition transform focus:outline-none ${isCartAnimating ? 'scale-[1.4] opacity-90 duration-500 ease-in-out' : 'hover:scale-105'}`}>
        <img
